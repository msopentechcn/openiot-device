...
int main(int argc, char** argv)
{
  ...
  char * msgdeviceid = (char *) "deviceID";
  opterr = 0;
  ...
  ppt = pn_message_properties(message);
  pn_data_put_map(ppt);
  pn_data_enter(ppt);
  pn_data_put_string(ppt, pn_bytes(strlen(msgdeviceid), msgdeviceid));
  pn_data_put_string(ppt, pn_bytes(strlen(msgtext), msgtext));
  pn_data_exit(ppt);
